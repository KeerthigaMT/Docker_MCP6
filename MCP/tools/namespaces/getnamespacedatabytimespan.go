...
func GetnamespacedatabytimespanHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		dataviewVal, ok := args["dataview"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: dataview"), nil
		}
		dataview, ok := dataviewVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: dataview"), nil
		}
		url := fmt.Sprintf("%s/namespaces/%s/pulls/exports/years/%s/%s/%s/%s", cfg.BaseURL, namespace, year, timespantype, timespan, dataview)
...
func CreateGetnamespacedatabytimespanTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_namespaces_namespace_pulls_exports_years_year_timespantype_timespan_dataview",
		mcp.WithDescription("Get namespace data for timespan"),
		...
		mcp.WithString("dataview", mcp.Required(), mcp.Description("Type of data to fetch")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetnamespacedatabytimespanHandler(cfg),
	}
}
